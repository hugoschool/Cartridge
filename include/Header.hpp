#pragma once

#include <cstdint>
#include <string>

namespace Cartridge {
    namespace HeaderValues {
        constexpr std::size_t romBytes = 4;
        constexpr std::size_t nintendoBytes = 156;
        constexpr std::size_t gameTitleBytes = 12;
        constexpr std::size_t gameCodeBytes = 4;
        constexpr std::size_t makerCodeBytes = 2;
        constexpr std::size_t reserved_1_bytes = 7;
        constexpr std::size_t reserved_2_bytes = 2;

        constexpr std::size_t fullSize = 192;

        constexpr std::uint8_t fixedValue = 0x96;
        constexpr std::uint8_t nintendoHeader[nintendoBytes] = {
            
        };
    }

    // https://problemkaputt.de/gbatek.htm#gbacartridgeheader
    struct HeaderContent {
        // 000h    4     ROM Entry Point  (32bit ARM branch opcode, eg. "B rom_start")
        std::uint8_t rom[HeaderValues::romBytes];

        // 004h    156   Nintendo Logo    (compressed bitmap, required!)
        std::uint8_t nintendo[HeaderValues::nintendoBytes];

        // 0A0h    12    Game Title       (uppercase ascii, max 12 characters)
        std::uint8_t gameTitle[HeaderValues::gameTitleBytes];

        // 0ACh    4     Game Code        (uppercase ascii, 4 characters)
        std::uint8_t gameCode[HeaderValues::gameCodeBytes];

        // 0B0h    2     Maker Code       (uppercase ascii, 2 characters)
        std::uint8_t makerCode[HeaderValues::makerCodeBytes];

        // 0B2h    1     Fixed value      (must be 96h, required!)
        std::uint8_t fixed;

        // 0B3h    1     Main unit code   (00h for current GBA models)
        std::uint8_t mainUnitCode;

        // 0B4h    1     Device type      (usually 00h) (bit7=DACS/debug related)
        std::uint8_t deviceType;

        // 0B5h    7     Reserved Area    (should be zero filled)
        std::uint8_t reserved_1[HeaderValues::reserved_1_bytes];

        // 0BCh    1     Software version (usually 00h)
        std::uint8_t softwareVersion;

        // 0BDh    1     Complement check (header checksum, required!)
        std::uint8_t complementCheck;

        // 0BEh    2     Reserved Area    (should be zero filled)
        std::uint8_t reserved_2[HeaderValues::reserved_2_bytes];
    };

    class Header {
        public:
            Header() = default;
            ~Header() = default;

            void readFromFile(const std::string fileName);
            static void generate(const std::string input, const std::string output, const std::string gameName);
            bool verify(bool print);

            class RestrictedString {
                public:
                    RestrictedString() = delete;
                    RestrictedString(std::size_t size, std::uint8_t bytes[]);
                    ~RestrictedString() = default;

                    std::string &getString();

                private:
                    std::size_t _size;
                    std::string _string;
            };

        private:
            HeaderContent _content;

            std::uint32_t getFullRom(std::uint8_t rom[HeaderValues::romBytes]) const;
            static std::uint8_t calculateComplementCheck(HeaderContent content);
            std::uint32_t littleToBigEndian(std::uint32_t) const;

            bool verifyNintendoLogo(std::uint8_t nintendo[HeaderValues::nintendoBytes]) const;
            bool verifyEmptyArea(std::uint8_t *reserved, std::size_t size) const;
    };
}
